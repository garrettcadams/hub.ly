			...
			if ( ! $product->is_in_stock() && in_array( 'outofstock', $badges ) ) {
				...
				if ( $product->get_type() == 'variable' ) {
					...
					for ( $i = 0; $i < count( $available_variations ); $i ++ ) {
						...
						$save       = $regular_price ? $regular_price - $sales_price : 0;
				...
				if ( martfury_get_option( 'sale_type' ) == '2' ) {
					if ( $save ) {
						$output[] = '<span class="onsale ribbon"><span class="sep">' . esc_html( martfury_get_option( 'sale_save_text' ) ) . '</span>' . ' ' . wc_price( $save ) . '</span>';
					}
				} else {
					if ( $percentage ) {
